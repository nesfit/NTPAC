...
{
  public class OnlineLoadBalancerCompleteTunnedActor : OnlineLoadBalancerBenchmarkActorBase
  {
    public OnlineLoadBalancerCompleteTunnedActor(LoadBalancerSettings settings, IPcapLoader pcapLoader) : base(
      settings, pcapLoader)
    {
    }

    public new static Props Props(LoadBalancerSettings settings, IPcapLoader pcapLoader) =>
      Akka.Actor.Props.Create(() => new OnlineLoadBalancerCompleteTunnedActor(settings, pcapLoader));
    ...
    protected override async Task MaterializePipeline(Source<RawCapture, NotUsed> packetSource, ActorSystem system)
    {
      ...
      var materializerSettings = ActorMaterializerSettings.Create(system).WithInputBuffer(1024, 1024);
