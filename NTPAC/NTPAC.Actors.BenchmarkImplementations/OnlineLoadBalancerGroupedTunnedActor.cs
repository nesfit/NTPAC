...
{
  public class OnlineLoadBalancerGroupedTunnedActor : OnlineLoadBalancerBenchmarkActorBase
  {
    public OnlineLoadBalancerGroupedTunnedActor(LoadBalancerSettings settings, IPcapLoader pcapLoader) : base(
      settings, pcapLoader)
    {
    }

    public new static Props Props(LoadBalancerSettings settings, IPcapLoader pcapLoader) =>
      Akka.Actor.Props.Create(() => new OnlineLoadBalancerGroupedTunnedActor(settings, pcapLoader));
    ...
    private Source<IEnumerable<RawPacket>, NotUsed> CreatePipeline(Source<RawCapture, NotUsed> packetSource) =>
      packetSource
        ...
        .Grouped(this.Settings.BatchSize).MergeSubstreams() as Source<IEnumerable<RawPacket>, NotUsed>;
  }
}
